...
function PlayerDisplay(board, player) {
	...
	const auto = !player
	if (player) player.addObserver(this)
	...
	this.notifyUpdates = function () {
		...
		for (let i of scores) html += '<tr><td>' + i.name + ':</td><td>' + i.pts + '</td></tr>'
		document.getElementById('scores').innerHTML = html

		// Board
		const max = this.halfWidth > this.halfHeight ? this.halfWidth : this.halfHeight
		for (let w = max; w > 0; --w) {
			const cells = [document.getElementById(idOf(w, w)), document.getElementById(idOf(-w, -w)), document.getElementById(idOf(-w, w)), document.getElementById(idOf(w, -w)), document.getElementById(idOf(w, 0)), document.getElementById(idOf(0, w)), document.getElementById(idOf(-w, 0)), document.getElementById(idOf(0, -w))]
		...
		for (let o in orientations) {
			...
			m(s[o][1], or, s[o][2])
			m(s[o][3], or, COIN)
			if (s[o][4] > 0) for (let i = s[o][4]; i <= max; i++) m(i, or, 'c_off')
		}

		const centerCell = document.getElementById(idOf(0, 0))
		centerCell.innerHTML = reduceName(player.name)
		for (let i = 3; i >= 0; --i) centerCell.classList.remove(ASTRO + i)
	...
	this.getHtml = function () {
		...
		for (let y = this.halfHeight; y >= -this.halfHeight; --y) {
			html += '<tr>'
			for (let x = -this.halfWidth; x <= this.halfWidth; x++) html += '<td id="' + idOf(x, y) + '" class="' + ((x === 0 || y === 0 || y === x || x === -y) ? 'cell' : 'c_off') + '"></td>'
			html += '</tr>'
		}
		return html
	}
	...
	this.changePlayer = function (newPlayer) {
		...
		if (player) player.remObserver(this)
		player = newPlayer
